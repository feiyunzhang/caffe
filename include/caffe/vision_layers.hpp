...
/* ROIMaskPoolingLayer - Region of Interest Pooling Layer With Mask
*/
template <typename Dtype>
class ROIMaskPoolingLayer : public Layer<Dtype> {
 public:
  explicit ROIMaskPoolingLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "ROIMaskPooling"; }
