...
void ROIMaskPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_rois; ++n) {
    ...
    int roi_start_w = round(xx1 * spatial_scale_);
    int roi_start_h = round(yy1 * spatial_scale_);
    int roi_end_w = round(xx2 * spatial_scale_);
    int roi_end_h = round(yy2 * spatial_scale_);
    ...
    int mask_start_w = round(mx1 * spatial_scale_);
    int mask_start_h = round(my1 * spatial_scale_);
    int mask_end_w = round(mx2 * spatial_scale_);
    int mask_end_h = round(my2 * spatial_scale_);
